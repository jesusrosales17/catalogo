...
const moreProduct = (product) => {
  //  aumenta en 1 el producto
  const dataProduct = dataProducts.find(
    (p) => p.idProducto === product.idProducto
  );
  const dataProductSold = JSON.parse(dataVenta.pedido).find(
    (sold) => sold.idProducto === dataProduct.idProducto
  );

  // productsSelected = productsSelected.map((p) => {
  //   if (
  //     p.idProducto === product.idProducto && // si el producto es el mismo
  //     (
  //       p.cantidadAVender < dataProduct.cantidadProducto || // y la cantidad a vender es menor a la cantidad total
  //       p.cantidadAVender === dataProduct.catidad
  //     )
  //   ) {
  //     p.cantidadAVender += 1;
  //   }
  ...
  productsSelected = productsSelected.map((p) => {
    if (p.idProducto === product.idProducto) {
      let availableStock = Number(dataProduct.cantidadProducto); // Cantidad disponible en stock
  ...
  productsSelected.forEach((product) => {
    ...
    fullSalePrice +=
      dataProducts.find((p) => product.idProducto === p.idProducto)
...
const showModal = (products) => {
  ...
  wayToPayInput.value = dataVenta.formaPago;
  document.querySelector("#clientInput").value = dataVenta.idCliente;
  document.querySelector("#resumNameClient").textContent = dataClients.find(
    (client) => client.idCliente === dataVenta.idCliente
  ).nombreCompleto;

  if (dataVenta.formaPago > 1) {
    document.querySelector("#paymentAmount").value = dataVenta.cantidadPagos;
  }
...
const getData = (id) => {
  ...
  const urlVenta = "http://sistema.test/api/ventas.php/" + id;
...
clientInput.addEventListener("change", (e) => {
  const dataClient = dataClients.find(
